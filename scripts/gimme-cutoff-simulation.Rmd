...
sim_generate <- function(condition, 
  ...
  # TODO check if I have to provide this as fixed object
  diag(a_1) <- rep(ar, 6)
  a_2 <- a_1 + change_a
  a_list <- list(a_1, a_2)
...
sim_summarize <- function(condition, results, fixed_objects = NULL){
  ...
  #----- Subgroup ------
  ari_sub <- lapply(seq_along(results[[m]]), function(i){
    
    if(is.null(results[[m]][[i]]$sub_ind)){
      NULL
    } 
    else{
      perturbR:::arandi(results[[m]][[i]]$dgp_sub_ind, results[[m]][[i]]$sub_ind)
    }
