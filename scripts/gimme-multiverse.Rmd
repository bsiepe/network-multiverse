...
```{r multiverse-personality}
# TODO CHANGE THIS TO THE NEW SPECIFICATION WITH .51
...
TODO this does not account for contemporaneous 
```{r}
...
# Added contemporaneous indicator manually
# result <- lapply(names(comp_pers$l_diff_adj_i[[1]]), function(matrix_name) {
#   add_matrices(lapply(comp_pers$l_diff_adj_i, function(element) element[[matrix_name]][,7:12]))
# })
...
beepr::beep(6)
# saveRDS(comp_emot, here("output/mv_emotion/comp_emot51.RDS"))
...
plot.new()
text(0.5,0.3,"Reference Network",cex=1.2,font=1)
# Temporal
qgraph::qgraph(
...
multiverse.network(comp_pers, cutoff = .05, title.cex = 1, label.cex = 1.21, n_ind = 94,
                   fade = TRUE)
