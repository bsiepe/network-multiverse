...
mv_res_emot <- readRDS(here("output/mv_emotion/mv_res_emot2.RDS"))
...
# Analyze
tic()
comp_emot <- multiverse.compare(mv_res_emot, ref_fit_emot, n_ind = 105)
toc() 
...
```{r}
g_75_51_emot <- mvgimme::multiverse.gimme()
...
Nonconvergence:
```{r}
summary(sapply(mv_res_emot, function(y){
  sum(unlist(lapply(y$path_est_mats, function(x) is.logical(x))))
}))
...
```{r}
# subtract the 6 AR paths
n_group_paths <- sapply(comp_emot$adjacency_g, function(x) sum(unlist(x))) - 10
summary(n_group_paths)
prop.table(table(n_group_paths))
```
Which group-level paths were found most often?
...
Number and size of subgruops
```{r}
summary(unlist(comp_emot$n_sub_g))
