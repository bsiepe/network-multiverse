...
mv_res_emot <- readRDS(here::here("output/mv_emotion/mv_res_emot_new51.RDS"))
...
The wording here is confusing: It should actually be called homogeneity if we want to interpret higher values as more homogeneous. 
Please see the revised version of the manuscript for details. 
```{r}
...
For the revision: Look at subgroup and group paths 
```{r}

# Obtain edge index of paths with group effects
group_path_index <-lapply(mv_res_emot, function(x){
  df_ft <- data.frame(from = x$group_plot_paths$Edgelist$from,
                      to = x$group_plot_paths$Edgelist$to,
                      weight = x$group_plot_paths$Edgelist$weight)
...
# Obtain edge index of paths with subgroup effects
sub_path_index <-lapply(mv_res_emot, function(x){
  which(x$group_plot_paths[["Arguments"]][["edge.color"]] %in% "green3")
}
)
...
# 
pdf("figures/ref_multi_net_pers_rev1.pdf")
par(mar=c(1,1,1,1))
...
multiverse.network(comp_pers, node.width = 2.5,
      node.height = 2.5,cutoff = .05, title.cex = 1, label.cex = 1.25, n_ind = 94,
                   edge.label.cex = 1.5, fade = TRUE, maximum = .15)
...
We create similar plots for a presentation.

For the network, only show the contemporaneous networks:
...
pct_labels_cont_ref <- paste0(round(t(ref_cont_mat)*100,1), " %")

pdf("figures/ref_multi_net_pers_presentation.pdf", width = 9, height = 4.5)
par(mar=c(1,1,1,1))
...
qgraph::qgraph(
      ...
      node.width = 1.5,
      node.height = 1.5,
      edge.labels  = pct_labels_cont_ref,
      theme = "colorblind",
      fade = TRUE,
      negDashed = TRUE,
      parallelEdge = TRUE,
...
pdf("figures/ref_multi_net_emot_rev1.pdf")
par(mar=c(1,1,1,1))
layout(matrix(c(1,2,3,5,1,4,3,6),ncol=2),heights=c(.5,3.5,.5,3.5))
plot.new()
text(0.5,0.3,"Reference Network",cex=1.3,font=1)
...
pdf("figures/ref_multi_net_emot_presentation.pdf", width = 9, height = 4.5)
par(mar=c(1,1,1,1))
layout(matrix(1:2, nrow = 1))  # 1 row, 2 columns
# text(0.5,0.3,"Reference Network",cex=1.3,font=1)
...
# insert zero for endogeneous variable
...
plot_out_adj_emot <- plot_outcome(mv_res = comp_emot, var = diff_adj_sum_mean_i,
  ...
  theme(axis.title = element_text(size = rel(1.5)),
        ...
        plot.margin = unit(c(0, 0, 0, 0), "cm"))+
          # add reference line
  geom_hline(yintercept = 0, color = "black", lty = 2)+
  scale_x_continuous(limits = c(0,3888), expand = c(0,0))
...
plot_spec_adj_emot <- plot_specification(mv_res = comp_emot, var = diff_adj_sum_mean_i)+
  theme(axis.title = element_text(size = rel(1.8)),
        ...
        # decrease spacing between facets
        panel.spacing = unit(0.5, "lines"),
        strip.text.y.left = element_text(size = rel(1)),
...
# Combine Plots
spec_plot_adj_emot <- cowplot::plot_grid(plot_out_adj_emot, plot_spec_adj_emot, 
                   nrow = 2, align = "h", axis = "bt",
                   rel_heights = c(0.5,1))
ggsave("spec_plot_adj_emot_presentation.pdf", spec_plot_adj_emot, path = here("figures/"),
       device = "pdf", height = 9, width = 14)
```


We create a similar plot for my doctoral thesis, where we want somewhat more readable labels: 

```{r}
#| label: spec-plot-adj-emot-thesis

sub_cuts <- c(.51, .60, .75, .80)

plot_out_adj_emot <- plot_outcome(mv_res = comp_emot,
                                  var = diff_adj_sum_mean_i,
                                  y_label = "Mean Adjacency Matrix Difference") +
  ...
  scale_x_continuous(
    limits = c(0, 4000),
    expand = c(0, 0),
    breaks = c(0, 1000, 2000, 3000, 3888)
  )
...
spec_plot_adj_emot <- cowplot::plot_grid(plot_out_adj_emot, plot_spec_adj_emot, 
                   nrow = 2, align = "h",
                   rel_heights = c(0.5,1))
ggsave("spec_plot_adj_emot_thesis.pdf", spec_plot_adj_emot, path = here("figures/"),
       device = "pdf", height = 16*0.9, width = 16*0.85)
```
