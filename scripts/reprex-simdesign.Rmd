---
title: "reprex-simdesign"
author: "Björn Siepe"
date: "`r Sys.Date()`"
output: html_document
---

This is a minimal reprex to reproduce the variable naming error with `SimDesign`. 
...
library(here)
set.seed(35037)
source(here::here("scripts/reprex-auxfuns.R"))
```
...
# Probability of individual contemporaneous effect
ind_a <- c(.15, .40)

# Probability of individual temporal effect
ind_phi <- c(.05, .25)
...
sim_analyse_gimme <- function(condition, dat, fixed_objects = NULL){
  ...
  gimme_default <- try(gimme(data = dat$dat,
                     ar = TRUE, 
                     subgroup = TRUE,
                     groupcutoff = 0.75,
                     subcutoff = 0.75))
  ...
  # Check if the gimme function call was successful
  # if (inherits(gimme_default, "try-error")) {
  #   # If there was an error, create a list with "NA" values for each element
  ...
  # } else {
    results$default <- list(
      path_counts = gimme_default$path_counts,
      path_est_mats = gimme_default$path_est_mats,
      sub_ind = gimme_default$fit$sub_membership,
  ...
  gimme_liberal <- try(gimme(data = dat$dat,
                     ar = TRUE, 
                     subgroup = TRUE,
                     groupcutoff = 0.50,
                     subcutoff = 0.50))
  
  # if (inherits(gimme_liberal, "try-error")) {
  #   # If there was an error, create a list with "NA" values for each element
  ...
  # } else {
    results$liberal <- list(
      path_counts = gimme_liberal$path_counts,
      path_est_mats = gimme_liberal$path_est_mats,
      sub_ind = gimme_liberal$fit$sub_membership,
      dgp_path_counts = dgp_path_counts,
      dgp_ind_paths = dgp_ind_paths,
      dgp_sub_ind = dgp_sub_ind
    )
...
```{r}
# For testing use first condition only
sim_design_test <- sim_design[1,]
replications = 3
